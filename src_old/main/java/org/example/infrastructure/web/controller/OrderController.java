...
public class OrderController {
    ...
    public ResponseEntity<OrderResponseDto> createInStockOrder(
        ...
        Car car = carQueryService.getCarById(carId);

        Order order = orderUseCase.createInStockOrder(client, manager, car);
    ...
    public ResponseEntity<OrderResponseDto> createCustomOrder(
        ...
        Order order = orderUseCase.createCustomOrder(client, manager, car);
        return ResponseEntity.status(HttpStatus.CREATED).body(webMapper.toDto(order));
    }
    ...
    private User getUserOrThrow(Jwt jwt) {
        ...
        if (user == null) {
            throw new UnauthorizedAccessException("Клиент не найден");
        }
        return user;
    }
    ...
    private User getManagerOrThrow() {
        return userRepository.findAll().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new RuntimeException("В системе нет менеджеров"));
    }
}